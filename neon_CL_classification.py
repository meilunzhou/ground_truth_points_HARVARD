...
import gc
import psutil
from TripletNetwork_Online import discrete_classification
...
from neon_custom_losses import keras_batch_all_triplet_continuous_loss_final, ciou_loss, dice_loss, ssim_loss, mean_squared_loss
...
def load_embedding_model(modality, run, cls_weight, patch_size):
    ...
    model = load_model(
        f'/blue/azare/zhou.m/commanet/ground_truth_points_HARVARD/trained_models/image_reconstruction/continuous_triplet/noL2/{modality}/class_10/patch_size_{patch_size}/_emb1024_batch32_margin0.1_test_size{test_size}_cls_weight{cls_weight}_best_val_model_run_{run}.h5',
        compile=False
    )
...
# Load the model for the current run
def load_mm_embedding_model(modality, run, cls_weight, patch_size):
    print('loading model for modality: ', modality, ' test_size: ', test_size, ' cls_weight: ', cls_weight, 'patch_size: ', patch_size)
    model = load_model(
        f'/blue/azare/zhou.m/commanet/ground_truth_points_HARVARD/trained_models/image_reconstruction/continuous_triplet/noL2/{modality}/v2/class_10/patch_size_{patch_size}/_emb1024_batch32_margin0.1_test_size{test_size}_cls_weight{cls_weight}_best_val_model_run_{run}.h5',
        compile=False
    )
    return model
...
if __name__ == "__main__":
    ...
    base_save_path = os.getcwd() + "/trained_models/discrete_class_label_classification/continuous_triplet/noL2/CCE/"
    ...
    for run in range(parameters['runs']):
        ...
        experiment_name = f"{modality_chosen}_CL_CLF_{run_number}"
        ...
        # Step 1: Encode class labels into integers
        ...
        # # Convert the array of integers to their corresponding string labels
        # train_box_labels_str = np.vectorize(box_label_mapping.get)(y_triplet_box_train)
        # test_box_labels_str = np.vectorize(box_label_mapping.get)(y_triplet_box_test)
        

#         embedding_model_multimodal = load_mm_embedding_model('multimodal', run_number)

#         train_embeddings_mm, _, _, _ = embedding_model_multimodal.predict([X_rgb_train, X_hsi_train, X_lidar_train])
#         val_embeddings_mm, _, _, _ = embedding_model_multimodal.predict([X_rgb_val, X_hsi_val, X_lidar_val])
...
#             val_embeddings = val_embeddings_mm[:, 2048:3072]
            ...
            # embedding_model_multimodal = load_mm_embedding_model('multimodal', run_number, parameters['cls_weight'], parameters['patch_size'])
...
#             train_embeddings, _, _, _ = embedding_model_multimodal.predict([X_rgb_train, X_hsi_train, X_lidar_train])
#             val_embeddings, _, _, _ = embedding_model_multimodal.predict([X_rgb_val, X_hsi_val, X_lidar_val])
        
        print('train_embeddings.shape: ', train_embeddings.shape)
        print('val_embeddings.shape: ', val_embeddings.shape)
        print('y_train_encoded.shape: ', y_train_encoded.shape, np.unique(y_train_encoded, return_counts=True))
        print('y_val_encoded.shape: ', y_train_encoded.shape, np.unique(y_val_encoded, return_counts=True))

    
        model = discrete_classification(EMB_DIM = train_embeddings.shape[1])
