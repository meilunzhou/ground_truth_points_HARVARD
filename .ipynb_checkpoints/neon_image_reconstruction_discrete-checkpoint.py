...
import gc
import psutil
from TripletNetwork_Online import image_reconstruction_triplet, image_reconstruction_triplet4x4, image_reconstruction_triplet6x6, image_reconstruction_triplet7x7, image_reconstruction_triplet11x11
...
from neon_custom_losses import keras_batch_all_triplet_continuous_loss_final, ciou_loss, dice_loss, ssim_loss, mean_squared_loss, hybrid_hsi_loss, rgb_ssim_loss
...
# }
    ...
    base_save_path = os.getcwd() + "/trained_models/image_reconstruction/discrete_triplet/noL2/"
    ...
    if modality_chosen == 'rgb':
        ...
        chosen_loss = rgb_ssim_loss(alpha=0.1)
        
    elif modality_chosen == 'lidar':
        S1_DIM = X_lidar_train[0].shape
        chosen_loss = mean_squared_loss
        
    elif modality_chosen == 'hsi':
        S1_DIM = X_hsi_train[0].shape
        chosen_loss = hybrid_hsi_loss
    ...
    for run in range(parameters['runs']):
        ...
        experiment_name = f"{modality_chosen}_discrete_triplet_image_recon_{run_number}"
        ...
        y_val_encoded = label_encoder.fit_transform(y_cls_val)
        y_test_encoded = label_encoder.fit_transform(y_cls_test)
...
#         model = image_reconstruction_triplet11x11(S1_DIM, modality=modality_chosen)
        
        model = image_reconstruction_triplet4x4(S1_DIM, modality=modality_chosen)
        
        model.compile(optimizer=Adam(0.0001), loss=[keras_batch_all_triplet_continuous_loss_final(margin=parameters['margin']), chosen_loss])
        ...
        if modality_chosen == 'rgb':
            ...
            log_gt_image_per_class(experiment, X_rgb_train, y_cls_train, step=0, train_val='train')
            log_gt_image_per_class(experiment, X_rgb_val, y_cls_val, step=0, train_val='val')
            
            for i in range(parameters['iterations']):

                model.fit(X_rgb_train, 
                           [y_train_encoded, X_rgb_train], 
                           validation_data = (X_rgb_val, 
                                                [y_val_encoded, X_rgb_val]), 
                ...
                if (i + 1) % 5 == 0:
                    ...
                    _, rgb_train_recon = model.predict(X_rgb_train)

                    log_one_image_per_class(experiment, rgb_train_recon, y_cls_train, step=i+1, train_val='train')

                    _, rgb_val_recon = model.predict(X_rgb_val)
                    log_one_image_per_class(experiment, rgb_val_recon, y_cls_val, step=i+1, train_val='val')
                    
        elif modality_chosen == 'lidar':
            print('Training for ', modality_chosen)
            # log_gt_image_per_class(experiment, X_lidar_train, y_cls_train, step=0, train_val='train')
            # log_gt_image_per_class(experiment, X_lidar_val, y_cls_val, step=0, train_val='val')
            
            for i in range(parameters['iterations']):

                model.fit(X_lidar_train, 
                           [y_train_encoded, X_lidar_train], 
                           validation_data = (X_lidar_val, 
                                                [y_val_encoded, X_lidar_val]), 
                           epochs = parameters['epochs'], 
                           batch_size=parameters['batch_size'], 
                           verbose=0, 
                           callbacks= [model_checkpoint_callback, comet_callback])
...
#                     _, lidar_train_recon = model.predict(X_lidar_train)

#                     log_one_image_per_class(experiment, lidar_train_recon, y_cls_train, step=i+1, train_val='train')

#                     _, lidar_val_recon = model.predict(X_lidar_val)
#                     log_one_image_per_class(experiment, lidar_val_recon, y_cls_val, step=i+1, train_val='val')
                    
        elif modality_chosen == 'hsi':
            print('Training for ', modality_chosen)
            
            # log_gt_hsi_per_class(experiment, X_hsi_train, y_cls_train, step=0, train_val='train')
            # log_gt_hsi_per_class(experiment, X_hsi_val, y_cls_val, step=0, train_val='val')
            
            for i in range(parameters['iterations']):

                model.fit(X_hsi_train, 
                           [y_train_encoded, X_hsi_train], 
                           validation_data = (X_hsi_val, 
                                                [y_val_encoded, X_hsi_val]), 
                           epochs = parameters['epochs'], 
                           batch_size=parameters['batch_size'], 
                           verbose=0, 
                           callbacks= [model_checkpoint_callback, comet_callback])
                ...
                if (i + 1) % 5 == 0:
                    ...
                    # _, hsi_train_recon = model.predict(X_hsi_train)

                    # log_one_hsi_recon_per_class(experiment, hsi_train_recon, y_cls_train, step=i+1, train_val='train')

                    # _, hsi_val_recon = model.predict(X_hsi_val)
                    # log_one_hsi_recon_per_class(experiment, hsi_val_recon, y_cls_val, step=i+1, train_val='val')
